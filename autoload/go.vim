function! go#open_test_pair()
  ...
  if l:file_name_wo_extension =~ "_test$"
    let l:file_to_open = l:file_path . '/' . l:file_name_wo_extension[:-6] . '.go'
  else
    let l:file_to_open = l:file_path . '/' . l:file_name_wo_extension . '_test.go'
  endif
  :silent! only
  :silent! vsplit
